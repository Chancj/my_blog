...
class MachineLearn:
    def __init__(self, sex, age, fare, alone):
        ...
        FILE_PATH = PATH + "data" + os.sep + "titainc.csv"  # 处理前的数据
        SAVE_PATH = PATH + "data" + os.sep + "ml.csv"  # 处理后的数据

        if not os.path.exists(SAVE_PATH):
            data_train = pd.read_csv(FILE_PATH)
            # 处理年龄空值
            # 获取称呼，根据称呼归类，填充平均值
            data_train["Initail"] = 0  # 新增一列，获取称呼
            data_train["Initail"] = data_train["Name"].str.extract('(\w+)\.')
